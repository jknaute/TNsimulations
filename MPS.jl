...
function LRcanonical_old(M,dir)
...
function MpoFromOperators(ops,L; fillZero=false)
    mpo = Array{Any}(L)
    d = size(ops[1][1])[1]
    for i = 1:L
        if fillZero # corresponds to 1-Id at positions
            mpo[i] = reshape(zeros(d,d),1,d,d,1)
        else
...
function qMPO(L, M, q=0)
    mpo = Array{Any}(L)
    mpo[1] = Array{Complex128}(1,2,2,2)
    mpo[1][1,:,:,:] = reshape([si M*cos(q)],2,2,2)
    mpo[L] = Array{Complex128}(2,2,2,1)
    mpo[L][:,:,:,1] = permutedims(reshape([M*cos(L*q) si], 2,2,2), [3,1,2])
    for i=2:L-1
        # hardcoded implementation of index structure (a,i,j,b):
        help = Array{Complex128}(2,2,2,2)
        help[1,:,:,1] = help[2,:,:,2] = si
...
function DMRG(mps_input, mpo, prec, orth=[]; max_iter=15)
    ...
    println("E, var = ", E, ", ", var)
    count=1
    while var > prec && count<max_iter
        Eprev = E
        mps, E, var, canonicity = sweep(mps,mpo,HL,HR,CL,CR,prec,canonicity,orth)
        println("E, var, prec = ", E, ", ", var, ", ",abs((Eprev-E)/E))
        count=count+1
...
function sweep(mps, mpo, HL, HR, CL, CR, prec,canonicity, orth=[])
    ...
    L = length(mps)
    N_orth = length(orth)
    orthTensors = Array{Array{Complex128,3},1}(N_orth)
    for j = 1:L-1
        if canonicity==-1
            j=L+1-j
        end
        ...
        function HeffFun(Avec)
            ...
            for k = 1:length(orth)
                ...
                Aout += 10*conj(orthTensors[k]) .* overlap
            end
            return vec(Aout)
        end
...
""" old version with projectors """
function sweep_old(mps, mpo, HL, HR, CL, CR, prec,canonicity, orth=[])
    ### minimizes E by diagonalizing site by site in the mps from left to right: j=1-->L-1
    ### the resulting sites are left-canonicalized
    L = length(mps)
    for j = 1:L-1
...
function n_further_states(mps, hamiltonian, prec, states, energies, n)
    for k = 1:n
        @time state,E = MPS.DMRG(mps,hamiltonian,prec,states)
        append!(states,[state])
        append!(energies,E)
...
""" absorp one mpo E into a mps site A to a new mps site A_new"""
function absorb_mpo_in_mps(A, E)
    @tensor A_new[-1,-2,-3,-4,-5] := A[-1,1,-4]*E[-2,1,-3,-5]
    sA = size(A_new)
    A_new = reshape(A_new, sA[1]*sA[2],sA[3],sA[4]*sA[5])
...
function makeCanonical_old(mps,n=0)
    L = length(mps)
    mpo_trafo = 0
    ...
    for i = 1:n-1
        mps[i],R,DB = LRcanonical_old(mps[i],-1);
        if i<L
...
function makeCanonical(mps,n=0)
    ...
    for i = L:-1:n+1
        ...
        if i>1
            @tensor mps[i-1][:] := mps[i-1][-1,-2,1]*R[1,-3]
            smps[i] = (size(R)[2],smps[i][2:4]...)
            smps[i-1] = (smps[i-1][1:3]..., size(R)[2])
        end
    end
